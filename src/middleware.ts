...
export function middleware(request: NextRequest) {
  ...
  if (EXCLUDED_PREFIXES.some((p) => pathname.startsWith(p)) || PUBLIC_FILE.test(pathname)) {
    ...
    // apps.* host accessing /apps-app directly (already-rewritten path):
    // let it through. Direct access on the MAIN host is handled below.
  ...
  // Host-based isolation for the apps subdomain. apps.* renders a dedicated,
  // locale-neutral design rewritten into the real /apps-app segment. This MUST
  // run before the [locale] logic so it bypasses it. Apps has no auth pages —
  // there is no APPS_PAGE_PREFIXES analog; all paths rewrite uniformly.
