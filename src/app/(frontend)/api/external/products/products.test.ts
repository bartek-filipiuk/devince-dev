...
// ── GET /api/external/products (list) ──────────────────────────────────────

describe('GET /api/external/products', () => {
  beforeEach(() => {
    process.env.EXTERNAL_API_TOKEN = TOKEN
    vi.clearAllMocks()
  })
  ...
  it('returns 200 with data.items', async () => {
    ...
    const res = await GET(makeGetReq('http://localhost/api/external/products'))
    expect(res.status).toBe(200)
    const body = await res.json()
    expect(body.data.items).toEqual([{ id: 1 }])
    expect(find).toHaveBeenCalledWith(expect.objectContaining({ collection: 'products' }))
  })

  it('returns 401 without a Bearer token', async () => {
    const { GET } = await import('./route.js')
    const res = await GET(makeGetReq('http://localhost/api/external/products', false))
    expect(res.status).toBe(401)
  })
})

// ── GET /api/external/products/[idOrSlug] ──────────────────────────────────

describe('GET /api/external/products/[idOrSlug]', () => {
  beforeEach(() => {
    process.env.EXTERNAL_API_TOKEN = TOKEN
    vi.clearAllMocks()
  })
  ...
  it('returns 200 with data.id for a numeric id', async () => {
    ...
    const res = await GET(makeGetReq('http://localhost/api/external/products/7'), {
  ...
  it('returns 404 for a missing slug', async () => {
    ...
    const res = await GET(makeGetReq('http://localhost/api/external/products/nope'), {
