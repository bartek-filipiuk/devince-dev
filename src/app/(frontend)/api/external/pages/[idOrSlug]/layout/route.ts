...
export async function PUT(
  ...
  try {
    ...
    const [pageId, layout] = await Promise.all([
      resolveDocId(payload, 'pages', idOrSlug, locale),
      convertBlocksMarkdown(body.layout as Record<string, unknown>[]),
    ])
    if (isErrorResponse(pageId)) return pageId
    ...
    return createSuccessResponse({
      ...
      title: page.title,
      slug: page.slug ?? '',
      blocksCount: Array.isArray(page.layout) ? page.layout.length : 0,
    })
  } catch (error) {
    return handleRouteError('Set page layout', error)
  }
}
