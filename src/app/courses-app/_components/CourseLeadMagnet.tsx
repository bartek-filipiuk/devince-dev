...
/**
 * Courses-surface wrapper around the shared LeadMagnetForm: gathers the i18n
 * copy (PL/EN) and renders the form with the `courses` surface so it posts to
 * /api/free-claim with the right surface and gets course-themed classes.
 */
export function CourseLeadMagnet({ slug, locale }: { slug: string; locale: Locale }) {
  return (
    <LeadMagnetForm
      surface="courses"
      slug={slug}
